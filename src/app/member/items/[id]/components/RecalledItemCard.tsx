...
interface RecalledItemCardProps {
  item: Item;
  cacheBust?: number;
}

export default function RecalledItemCard({ item, cacheBust }: RecalledItemCardProps) {
  ...
  return (
    <Card className="w-full overflow-hidden" variant="borderless">
      ...
      <CardContent className="p-8">
        ...
        {listingInfo && (
          <>
            <div className="p-4 bg-gray-50 rounded-lg text-sm border border-gray-100 shadow-sm mb-4">
              <div className="flex items-start gap-2">
                <AlertCircle className="h-5 w-5 mt-0.5 text-destructive/50" />
                <div className="flex-1 space-y-3">
                  <p className="font-medium">Item recalled</p>
                  ...
                  {listingInfo.recalled_at && (
                    <p className="text-sm text-muted-foreground">
                      Recalled on {formatDate(listingInfo.recalled_at)}
                    </p>
                  )}

                  {(listingInfo.reason || listingInfo.description) && (
            ...
            {(listingInfo.collection_deadline || listingInfo.collection_pin) && (
              <div className="p-4 bg-gray-50 rounded-lg text-sm border border-gray-100 shadow-sm mb-6">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    {listingInfo.collection_deadline && (
                      <p className="text-sm text-muted-foreground">
                        Must be collected by {formatDate(listingInfo.collection_deadline)}
                      </p>
                    )}
